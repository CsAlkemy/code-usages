...
// macOS gets native vibrancy (set on the BrowserWindow); flag it so the CSS
...
function fitWindow() {
  requestAnimationFrame(() => {
    ...
    // Glass mode has no body padding around the card; CSS mode keeps 10px
    // on each side for the drawn drop shadow.
...
// Initial pull (covers the case where the ready-time push already fired).
(async () => {
  try {
